use std::{thread::{JoinHandle, self}, sync::mpsc::{self, Receiver, Sender}};

use eframe::epaint::Color32;
use eskom_se_push_api::{area_search::{AreaSearch, AreaSearchURLBuilder}, errors::{HttpError, APIError}, Endpoint, area_info::{AreaInfo, AreaInfoURLBuilder}};

use crate::{traits::Page, ActivePage, StateData, helpers::map_error};
...
impl Page for FindAreaPage{
    fn page(&mut self, ui: &mut eframe::egui::Ui, state: &mut crate::StateData) {
        ...
        ui.add_enabled_ui(!self.search_term.trim().is_empty() && self.search_term.len() > 3, |ui| {
            ...
            search.areas.iter().for_each(|s| {
                ui.horizontal_top(|ui| {
                    ...
                    if ui.button("Add region").clicked() {
                        ...
                        let api_key = state.api_key.clone();
                        thread::spawn(move || {
                            let details = AreaInfoURLBuilder::default().area_id(id.clone())
                            .build().unwrap();
                            match details.reqwest(&api_key) {
                                Ok(t) => {tx.send(crate::SelectedAreaInfo { id: id, details: t });},
                                Err(e) => {err_tx.send(e);}
                            }
                        });
                    }
